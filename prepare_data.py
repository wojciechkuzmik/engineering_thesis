...
def prepare_data(filename, print_corr=False):
    ...
    def price_similarity(first, second):
        if float(first) > float(second):
            return 1 - (float(first) - float(second)) / float(first)
        else:
            return 1 - (float(second) - float(first)) / float(second)
    ...
    def engineer_features(data_frame):
        ...
        data_frame['brand_token_set_ratio'] = data_frame.apply(
            lambda x: fuzz.token_set_ratio(str(x['first_brand']),
                                 str(x['second_brand']))/100, axis=1)
        ...
        data_frame['title_log_fuzz_score_numbers'] = (data_frame['title_log_fuzz_score'] + (
            data_frame['title_matching_numbers']).apply(np.log))/100
    ...
    X = df[['title_token_set_ratio', 'title_log_fuzz_score_numbers', 'title_partial_ratio',
            ...
            'brand_token_set_ratio', 'price_similarity']].values
